...
/* ---------- Name matching ----------
   The website and the POS spell things differently: the site says
   "Injera (Regular)", Clover might say "Injera Regular" or just "Injera".
   Rather than let a real dish read as sold out, fall back through progressively
   looser keys. This can never mis-bill: the price still comes from Clover, and
   if the matched item's price differs from what the guest was shown the order is
   rejected by the total check below rather than charged. */
function normKey(s) {
  return String(s).toLowerCase().replace(/[^a-z0-9]+/g, "");
}
function baseKey(s) {
  return normKey(String(s).replace(/\([^)]*\)/g, " "));
}

/* norm + base indexes over the price book. First spelling wins, so an exact
   Clover name is never shadowed by a looser alias of another item. */
function aliasIndex(book) {
  const norm = {}, base = {};
  for (const k in book) {
    const entry = book[k];
    const n = normKey(entry.name), b = baseKey(entry.name);
    if (n && !(n in norm)) norm[n] = entry;
    /* Ambiguity is fatal, not first-wins: Clover has both "Injera" and "Teff
       Injera", and silently picking one would bill the wrong price. A base key
       claimed by two different items is poisoned so the loose match skips it. */
    if (b) base[b] = (b in base && base[b] && base[b].id !== entry.id) ? null : entry;
  }
  return { norm, base };
}

function lookup(book, idx, name) {
  return book[name.trim().toLowerCase()] ||
    idx.norm[normKey(name)] ||
    idx.base[baseKey(name)] ||
    idx.norm[baseKey(name)] || null;
}
...
/* An order with no orderType is a second-class citizen on the POS: it can fail to
   surface in the Orders app and gives the kitchen no routing. Prefer the
   merchant's own to-go/pickup type, fall back to whatever exists. */
async function orderTypeId(env) {
  try {
    const t = await clover(env, "/v3/merchants/" + env.CLOVER_MERCHANT_ID + "/order_types");
    const list = (t.elements || []).filter((x) => x && !x.isHidden && !x.isDeleted);
    const label = (x) => String(x.label || x.labelKey || "");
    const pick = list.find((x) => /to\s*go|take\s*-?\s*out|pick\s*-?\s*up|online|web/i.test(label(x))) || list[0];
    return pick ? pick.id : null;
  } catch (e) {
    console.error("order_types lookup failed:", String(e.message || e));
    return null;
  }
}
...
async function writeOrder(env, lines, totals, meta, chargeInfo) {
  ...
  const otype = await orderTypeId(env);

  /* The dish list goes in the order note as well as on the line items. If the
     line-item write is ever refused, the ticket still tells the kitchen what to
     cook rather than printing a nameless order. Clover truncates long notes, so
     cap it — the line items carry the authoritative list. */
  ...
  for (const l of lines) {
    for (let i = 0; i < l.qty; i++) {
      /* Name and price only — deliberately NOT the inventory id.
         Posting item.id made Clover link the line to inventory and then value it
         at 0, which left the itemized order at $0.00 while the real money sat on
         Clover's separate payment record. With no id there is nothing for Clover
         to resolve against: the name and price we send are the line, so the
         ticket reads correctly and the order total matches the card.
         Price authority is unchanged — these came from Clover's own price book
         above and were re-checked against the amount charged. */
      units.push({ name: l.name, price: l.price });
    }
  }
  ...
  /* Best effort: the money is already captured, so a failure here must never
     ...
     This is what stops the sale being counted twice: without it the itemized
     order reads unpaid and staff can charge the guest again at the counter. */
  try {
  ...
  /* Creating an order over REST does NOT print it. Clover routes a print_event to
     the firing device's order printer (or its onboard printer), and without this
     call the ticket simply never reaches the kitchen — which is exactly how a
     paid order goes silently missing. Best effort: the guest is already charged,
     so a print failure is a staff problem, not a checkout error. */
  try {
    await clover(env, mid + "/print_event", {
      method: "POST",
      body: JSON.stringify({ orderRef: { id: order.id } })
    });
    order.__printed = true;
  } catch (e) {
    order.__printed = false;
    console.error("PRINT FAILED for order " + order.id + " (" + meta.ticket + "):", String(e.message || e));
  }
...
async function handleOrder(request, env, ctx, head) {
  ...
  for (const raw of items) {
    ...
    const found = lookup(book, idx, nm);
  ...
  if (claimed && Math.abs(claimed - total) > 1) {
    /* Name the real number. "Prices changed" with no figure leaves the guest
       guessing and gives us nothing to debug from. */
    return fail(
      "The total has changed to $" + (total / 100).toFixed(2) +
      ". Please reopen your cart to see the current prices.",
...
async function selftest(env) {
  ...
  /* Order type decides which printer the ticket routes to. A web order that
     lands as "Dine In" is not just mislabelled — it can print to the wrong
     station or nowhere. Reported here because a silent fallback to Clover's
     default is invisible from the order itself. */
  try {
    const t = await clover(env, mid + "/order_types");
    const list = (t.elements || []).filter((x) => x && !x.isHidden && !x.isDeleted);
    const chosen = await orderTypeId(env);
    const match = list.find((x) => x.id === chosen);
    out.orderTypes = { ok: true, count: list.length,
      willUse: match ? (match.label || match.labelKey) : null,
      labels: list.map((x) => x.label || x.labelKey) };
  } catch (e) {
    out.orderTypes = { ok: false, error: String(e.message || e).slice(0, 300) };
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    /* /find?q=injera — every Clover item whose name contains q, with its exact
       spelling and price. The fastest way to see what the POS actually calls a
       dish when a checkout says it isn't available. Diagnostic only. */
    if (pathname === "/find") {
      if (!env.CLOVER_TOKEN || !env.CLOVER_MERCHANT_ID) return json({ error: "not configured" }, 500, head);
      try {
        const q = (searchParams.get("q") || "").trim().toLowerCase();
        const d = await clover(env, "/v3/merchants/" + env.CLOVER_MERCHANT_ID + "/items?limit=1000");
        const hits = (d.elements || [])
          .filter(i => i && i.name && !i.hidden && i.available !== false)
          .filter(i => !q || i.name.toLowerCase().includes(q))
          .map(i => i.name + "  $" + ((i.price || 0) / 100).toFixed(2))
          .sort();
        return json({ q, count: hits.length, items: hits }, 200, head);
      } catch (e) {
        return json({ error: String(e.message || e).slice(0, 400) }, 500, head);
      }
    }

    /* /recent — the last few orders as Clover holds them, so we can tell a POS
       write that silently failed from one that landed but never printed.
       Diagnostic only. */
    if (pathname === "/recent") {
      if (!env.CLOVER_TOKEN || !env.CLOVER_MERCHANT_ID) return json({ error: "not configured" }, 500, head);
      try {
        const mid = "/v3/merchants/" + env.CLOVER_MERCHANT_ID;
        const d = await clover(env, mid + "/orders?limit=10&expand=lineItems,orderType");
        const orders = (d.elements || []).map((o) => ({
          id: o.id,
          title: o.title || null,
          state: o.state || null,
          total: (o.total || 0) / 100,
          type: (o.orderType && (o.orderType.label || o.orderType.labelKey)) || null,
          created: o.createdTime ? new Date(o.createdTime).toISOString() : null,
          items: ((o.lineItems && o.lineItems.elements) || []).map((l) => l.name),
          note: o.note || null
        }));
        return json({ count: orders.length, orders }, 200, head);
      } catch (e) {
        return json({ error: String(e.message || e).slice(0, 400) }, 500, head);
      }
    }
    ...
    /* The bare root used to answer "not found", which reads as a broken worker to
       anyone checking on it. Serve a small index of the diagnostic routes instead
       — Safari hides URL paths, so a clickable list beats typing one. */
    if (pathname === "/" && request.method === "GET") {
      const link = (href, label, note) =>
        '<li style="margin:0 0 14px"><a href="' + href + '" style="color:#C7452B;font-weight:700;text-decoration:none">' +
        label + '</a><div style="color:#6b625c;font-size:14px;margin-top:2px">' + note + '</div></li>';
      return new Response(
        '<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
        '<title>Banna order API</title>' +
        '<body style="margin:0;padding:32px;background:#FAF7F3;color:#1B1512;' +
        'font:16px/1.5 ui-sans-serif,system-ui,-apple-system,sans-serif">' +
        '<h1 style="font-size:22px;margin:0 0 4px">Banna order API</h1>' +
        '<p style="color:#6b625c;margin:0 0 24px">Running. Checkout posts to <code>/order</code>.</p>' +
        '<ul style="list-style:none;padding:0;margin:0;max-width:560px">' +
        link("/health", "/health", "Is the worker configured and which Clover environment") +
        link("/recent", "/recent", "The last 10 orders as Clover holds them") +
        link("/find?q=side%20order", "/find?q=…", "Clover's exact name and price for anything") +
        link("/selftest", "/selftest", "Every Clover call the order path depends on") +
        '</ul></body>',
