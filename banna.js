...
(function () {
  ...
  /* ---------- Clover online payment ----------
     KEY and MID are public by design: the key only lets Clover's iframe accept a
     card, and the MID is on every receipt. The PRIVATE token is not here and must
     never be — it lives only in the Worker's secret store (see /api/README.md).
  ...
  /* ---------- nav dropdown ----------
     Closes on outside click, on tapping one of its own links (otherwise a
     same-page anchor leaves it hanging open, and the next tap on "More" only
     closes it, which reads as a dead button), and on a back-button restore,
     where Safari brings the page back with the panel still open. */
  document.addEventListener('click', function (e) {
