...
(function () {
  ...
  class BannaCart extends HTMLElement {
    ...
    addonStrip() {
      var open = this.addon;
      var self = this;
      function section(key, label, sub, list) {
        var on = open === key;
        var loading = key === "drinks" && DRINKS_STATE === "loading";
        return '<button type="button" class="dtab" data-tab="' + key + '" data-on="' + on + '" aria-expanded="' + on + '">' +
          '<span class="dtl"><strong>' + label + '</strong><span>' + (loading ? "Loading&hellip;" : sub) + '</span></span>' +
          '<span class="dtc" aria-hidden="true">' + (on ? "&minus;" : "+") + '</span></button>' +
          (!on ? "" : '<div class="drow">' + (loading
            ? '<span class="dskel"></span><span class="dskel"></span><span class="dskel"></span>'
            : list.map(function (d) {
                return '<button type="button" class="dchip" data-addon="' + esc(d.name) + '" data-kind="' + key + '">' +
      ...
      return '<div class="drinks">' +
        section("sides", "Extra sides", SIDES.length + " items &middot; injera, gomen, shiro", SIDES) +
        section("drinks", "Drinks", "Coffee, tea, water and sodas", DRINKS) +
        '</div>';
    }
    ...
    commit(open) {
      ...
      if (!this.count() && this.step !== "done" && this.step !== "cart") { this.openState = false; this.step = "cart"; }
    ...
    open() { this.step = "cart"; this.commit(true); }
    close() {
    ...
    body() {
      ...
      if (this.step === "cart") {
        if (!this.count()) {
          return '<div class="blank">Your order is empty. Add extra sides or a drink below, or ' +
            '<a href="https://banna-restaurant.cloveronline.com/menu/all" target="_blank" rel="noopener">browse the full menu</a>.</div>' +
            this.addonStrip() +
            '<button type="button" class="quiet" data-close>Keep browsing</button>';
        }
    ...
    render() {
      ...
      var top = !n && !done ?
        '<button type="button" class="fab" data-open aria-label="Open your order">' +
        '<span class="fabicon" aria-hidden="true">' +
        '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">' +
        '<path d="M4 8h16l-1.3 12H5.3L4 8Z"/><path d="M9 8V6a3 3 0 0 1 6 0v2"/></svg></span><span>Order drinks &amp; sides</span></button>'
        : (!n ? "" :
      ...
      this.root.innerHTML = '<style>' + BannaCart.css + '</style>' + top + sheet;
      ...
      this.root.querySelectorAll("[data-tab]").forEach(function (b) {
        b.onclick = function () {
          ...
          self.addon = self.addon === k ? null : k;
          self.render();
        };
      });
      ...
      if (form) form.onsubmit = function (e) {
        ...
        /* TEMPORARY: until the Clover payment backend is connected, reaching the payment
           step is our best available signal of an online purchase, so it fires the
           in-house order conversion here. Move this back to the confirmed-order path
           (see the [data-pay] handler above) once real payments land. */
  ...
  BannaCart.css = [
    ...
    ".fab{position:fixed;right:clamp(12px,4vw,24px);bottom:calc(18px + env(safe-area-inset-bottom));z-index:80;display:inline-flex;align-items:center;gap:10px;min-height:56px;padding:0 24px;border:0;border-radius:100px;background:" + RED + ";color:#FFF;font:800 16px 'Archivo',system-ui,sans-serif;cursor:pointer;box-shadow:0 10px 26px rgba(27,21,18,.32)}",
    ".fab:hover{background:" + RED_D + "}",
    ".fabicon{display:flex;line-height:1}",
    ".blank{font:600 14px/1.6 'Archivo',system-ui,sans-serif;color:rgba(27,21,18,.7);background:#FFF;border:1px solid rgba(27,21,18,.12);border-radius:14px;padding:16px;margin-bottom:14px;text-wrap:pretty}",
    ".blank a{color:" + RED_D + "}",
    "@media (max-width:520px){.fab{left:clamp(12px,4vw,24px);justify-content:center}" +
      ".bar{flex-direction:column;align-items:stretch;gap:9px}" +
      ".bmeta{flex-direction:row;align-items:center;justify-content:space-between;gap:10px}" +
      ".bsub{white-space:normal;text-align:right}" +
      ".bview{width:100%}" +
      ".spacer{height:132px}}",
    ...
    ".x{width:44px;height:44px;flex-shrink:0;border-radius:50%;border:1px solid rgba(27,21,18,.14);background:#fff;color:" + INK + ";font:700 18px 'Archivo',system-ui,sans-serif;cursor:pointer}",
    ...
    ".dtab{display:flex;align-items:center;justify-content:space-between;gap:12px;width:100%;text-align:left;min-height:58px;padding:11px 14px;margin-bottom:8px;border-radius:14px;border:1px solid rgba(27,21,18,.18);background:" + CREAM + ";cursor:pointer}",
    ".dtl{display:flex;flex-direction:column;gap:2px;min-width:0}",
    ".dtl strong{font:800 14.5px 'Archivo',system-ui,sans-serif;color:" + INK + "}",
    ".dtl span{font-weight:600;font-size:12px;color:rgba(27,21,18,.55)}",
    ".dtc{flex:0 0 auto;display:flex;align-items:center;justify-content:center;width:30px;height:30px;border-radius:50%;background:" + PANEL + ";color:" + INK + ";font:800 18px 'Archivo',system-ui,sans-serif;line-height:1}",
    ".dtab[data-on=\"true\"]{background:" + INK + ";border-color:" + INK + "}",
    ".dtab[data-on=\"true\"] .dtl strong{color:#F6EBD9}",
    ".dtab[data-on=\"true\"] .dtl span{color:rgba(246,235,217,.7)}",
    ".dtab[data-on=\"true\"] .dtc{background:rgba(246,235,217,.16);color:#F6EBD9}",
    ".dtab[data-on=\"true\"]+.drow{margin:-2px 0 10px}",
    ...
    ".quiet{display:flex;align-items:center;justify-content:center;width:100%;min-height:46px;margin:12px 0 0;padding:12px 16px;border-radius:100px;background:none;border:1px solid rgba(27,21,18,.18);font:700 13.5px 'Archivo',system-ui,sans-serif;color:rgba(27,21,18,.62);cursor:pointer}",
    ".quiet:hover{background:" + PANEL + ";color:" + INK + "}",
