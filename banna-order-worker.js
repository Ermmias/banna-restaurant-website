...
async function selftest(env) {
  ...
  try {
    ...
    const list = t.elements || [];
    const ext = list.find((x) => /external/i.test(x.labelKey || "") || /external/i.test(x.label || ""));
    out.tenders = { ok: true, count: list.length, externalTender: ext ? (ext.label || ext.labelKey) : null,
